...
def regression(request):
    ...
    try:
        ...
        def modeleval(y_test,Y_pred,model,name,df,sub):
            ...
            data_dict.update({"title4":"{} evaluation of the model on unseen data".format(name)})
        ...
        def Regress(x,y,subject,reg_name):
            ...
            data_dict.update({"title1":reg_name+" evaluation of the model on training data"})
        ...
        return HttpResponse("<h1>Some Error has been acquired</h1>")

def on_change(request):
    reg_mod=request.POST.get("reg_mod")
    print(reg_mod)
    if (reg_mod!=None):
        print("done")
        regression_name.update({"reg_name":reg_mod})
